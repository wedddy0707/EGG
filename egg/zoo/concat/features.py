...
class _OneHotIterator:
    ...
    def __next__(self):
        ...
        batch_data = self.random_state.multinomial(
            1, self.probs, size=(self.batch_size, self.n_inputs))
...
class UniformLoader(torch.utils.data.DataLoader):
    def __init__(self, n_features, n_inputs):
        eye = torch.stack([torch.eye(n_features)], dim=1)
        batch = eye
        ...
        self.batch = batch, torch.zeros(1)
